# Data Transformation and Python-Based User Menu Developement Project
# Overview
The purpose of this data engineering project is to provide the opportunity to demonstrate and combine the knowledge and skills I have acquired throughout this course.  
...
The following technologies that were used in this project were: Python (Pandas, advanced modules, Matplotlib), MariaDB, Apache Spark (Spark Core, Spark SQL), and Python Visualization and Analytics libraries.  
...
## STEP 1: Extract
I began by importing findspark, pyspark, and pandas so I can read/extract the above JSON files. Next I created a spark session, this can be used to create a dataframe, register dataframes as tables, and execute SQL over tables. I used spark to read json files into spark dataframe.

## STEP 2: Transform
Since I wanted to display my knowledge of Pandas and Apache Spark.  I converted CDW_SAPP_CUSTOMER.JSON and CDW_SAPP_CREDITCARD.JSON into a pandas dataframe to then transform this dataset according to the specifications found in the mapping document that was provided to me. For CDW_SAPP_BRANCH.JSON I used spark to transform the dataset according the mapping document.

## STEP 3: LOAD
Once json files were read/extracted into dataframe and transform I then loaded the data into MariaDB which is a RDBMS. First I created a database in SQL(MariaDB) named "creditcard_capstone".  Then used pyspark to load/write the "Credit Card System Data" into RDBMS database.
